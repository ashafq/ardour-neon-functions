 ...
 *
 * Defined as @c sizeof(__m128), i.e. 16 bytes.
 */
#define N_ALIGNMENT sizeof(__m128)

/**
 * @def N_SIMD
 * @brief Number of single-precision floats processed per AVX vector register.
 *
 * For AVX @c __m128 this is 4 floats.
 */
 ...
 */
C_FUNC float
x86_sse_compute_peak(const float* src, uint32_t nframes, float current)
{
	...
	const __m128 fp_sign = _mm_set1_ps(-0.F);
	__m128 xmax = _mm_set1_ps(current);
	...
	if (simd_count >= 4)
	{
		...
		for (size_t i = 0; i < unrolled_count; ++i)
		{
			...
			__m128 x0, x1, x2, x3;
			__m128 max0, max1, max2;

			// Load data from memory
			x0 = _mm_load_ps(ptr + (0 * N_SIMD));
			x1 = _mm_load_ps(ptr + (1 * N_SIMD));
			x2 = _mm_load_ps(ptr + (2 * N_SIMD));
			x3 = _mm_load_ps(ptr + (3 * N_SIMD));

			// Compute absolute values
			x0 = _mm_andnot_ps(fp_sign, x0);
			x1 = _mm_andnot_ps(fp_sign, x1);
			x2 = _mm_andnot_ps(fp_sign, x2);
			x3 = _mm_andnot_ps(fp_sign, x3);

			// Compute the peaks
			max0 = _mm_max_ps(x0, x1);
			max1 = _mm_max_ps(x2, x3);
			max2 = _mm_max_ps(max0, max1);

			xmax = _mm_max_ps(xmax, max2);
		}
	...
	for (size_t i = start; i < simd_count; ++i)
	{
		...
		x = _mm_load_ps(src_aligned + offset);
		x = _mm_andnot_ps(fp_sign, x);
		xmax = _mm_max_ps(xmax, x);
	}
...
reduce_max_ret:
	current = x86_sse_reduce_max(xmax);
